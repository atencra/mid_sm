function [infodata] = sm_info_fraction(xbins, x, locator, fraction, x2bins, x2)
    ...
    for i = 1:length(x)
        ...
        for j = 1:length(fraction)
            ...
            for k = 1:nreps
                ...
                iplugin_reps(k) = sm_info_px_pxspk(px, pxspk);
    ...
    for i = 1:length(x)
        ...
        for j = 1:length(fraction)
            ...
            for k = 1:nreps
                ...
                nx1x2 = sm_hist2d(x1x2, center2edge(xbins{i}), center2edge(x2bins{i}));
                px1x2 = nx1x2 ./ sum(sum(nx1x2)); % normalize

                nx1x2spk = sm_hist2d(x1x2spk,  center2edge(xbins{i}), center2edge(x2bins{i})); 
                px1x2spk = nx1x2spk ./ sum(sum(nx1x2spk)) + eps; % normalize

                iplugin_reps(k) = sm_info_px_pxspk(px1x2, px1x2spk);
