function [ifrac] = sm_info_fraction_2d(locator, x1, x2, x1binedges, x2binedges, frac)
...
for i = 1:nreps
   ...
   nx1x2 = sm_hist2d(x1x2, x1binedges, x2binedges);
   px1x2 = nx1x2 ./ sum(sum(nx1x2)); % normalize

   nx1x2spk = sm_hist2d(x1x2spk, x1binedges, x2binedges);
   px1x2spk = nx1x2spk ./ sum(sum(nx1x2spk)) + eps; % normalize


   % Now calculate the 2D information:

   iplugin = sm_info_px_pxspk(px1x2, px1x2spk);
