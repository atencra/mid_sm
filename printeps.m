...
%printeps  Find and replace Matlab default fonts in eps files
%
...
%    4/20/05
%
%    Modified by Craig Atencio, 1/1/2009, to only read and replace a
%    font in a file. The function no longer prints a figure to an eps
%    file. The function finds the matlab default fonts and replaces 
%    this with the 'Arial' font.
%
...
if ( ~isempty(findstr(figname, '.eps')) )
	figfilestr = [figname];
else
...
%get the actual font
figure(fignum);
actualfont = get(gca,'FontName'); % the font that is present on the figure
actualfont = 'Arial';
...
for k = 1:length(mlabfontlist)
   ff = strrep(ff,mlabfontlist{k},actualfont); % replace actual w/ desired font
end
