function cmap = brewmaps(map, nColors)
% brewmaps  Color Brewer colormaps for graphics
%     cmap = brewmaps(map, N) returns an RGB colormap specified 
...
%     in matlab then applies the desired colormap. Issuing brewmaps without 
%     any input or output arguments will display the colormaps in one
%     figure.
% 
...
if ( nargout == 0 && nargin == 0 ) % show all the colormaps
...
function show_brewer_maps
% Plots and identifies the available various colorbrewer tables.
% Is called by cschemes.m when no arguments are given.
%
% Author: Charles Robert
% email: tannoudji@hotmail.com
% Date: 14.10.2011
%
% Modified by Craig Atencio for use with his function that plots the
% Color Brewer color maps.
...
ctypes={'div', 'seq'};
ctypes_title={'Diverging', 'Sequential'};
cnames{1,:}= divmaps;
cnames{2,:}= seqmaps;
...
for itype=1:2
   ...
   for iname=1:length(cnames{itype,:})
      ...
      if ( itype == 1 )
         cmap = diverging_maps( cnames{itype}{iname});
      else
         cmap = sequential_maps( cnames{itype}{iname});
      end
...
% This function cycles through the maps one by one, using the matlab
% logo to show the effect. Presently, the subfunction show_brewer_maps
% is used instead to show the maps.
...
% RGB triples for Color Brewer Diverging maps
...
cmap = flipud(x)/256; % the flip makes the maps ready to use for STRFs
...
% RGB triples for Color Brewer Sequential maps
...
cmap = flipud(x)/256; % the flip makes the lowest value correspond to the
                      % lightest color
